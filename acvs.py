...
# Define features target and input column lists
features_target_col = ['eges']
features_input_col = df_raw.columns.tolist()
if 'eges' in features_input_col:
    features_input_col.remove('eges')
...
df = pd.DataFrame(df_raw, columns=['eges'] + features_col)
df_d = pd.get_dummies(df[features_col])
st.dataframe(df_d.head(10))

st.markdown("features with correlation to HIGH eges:")
st.write(df_d.corrwith(df["eges"]).sort_values(ascending=False)[:5])
st.markdown("features with correlation to LOW eges:")
st.write(df_d.corrwith(df["eges"]).sort_values()[:5])
...
fig3 = sns.catplot(x=feature_studied, y=target_name, hue=hue_col, kind="swarm", data=df, height = 10)
st.pyplot(fig3, use_container_width=True)
fig4 = sns.catplot(x=feature_studied, y=target_name,  kind="violin", data=df, height = 8, aspect=2)
